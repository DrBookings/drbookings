...
public class FixkostenAnteilCalculatorTest2018June {
	...
	final MonetaryAmount anteil = new DefaultFixedCostsAnteilCalculator(month, data.getBookings())
		.getFixkostenAnteil("Booking", true, Payments.getSum(History.getCommonExpenses2018June()));
	assertThat(anteil.with(Monetary.getDefaultRounding()), is(Payments.createMondary(1438.49)));
    }

    @Test
    public void testAirbnb01() {
	final MonetaryAmount anteil = new DefaultFixedCostsAnteilCalculator(month, data.getBookings())
		.getFixkostenAnteil("Airbnb", true, Payments.getSum(History.getCommonExpenses2018June()));
	assertThat(anteil.with(Monetary.getDefaultRounding()),
		is(Payments.createMondary(609.5297619).with(Monetary.getDefaultRounding())));
    }
