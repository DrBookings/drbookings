...
public class NumberOfNightsCounterTest2018July {
	...
	final long nights = new NumberOfNightsCounter(month,
		new BookingsByOrigin<>(data.getBookings()).getAirbnbBookings()).setIgnorePaymentDate(true)
			.countNights();
	assertThat(nights, is(5L + 2 + 5 + 4 + 3 + 3));
    }
	...
	assertThat(nights, is(111L));
	nights = new NumberOfNightsCounter(month, new BookingsByOrigin<>(data.getBookings()).getAllBookings(false))
		.setIgnorePaymentDate(true).countNights();
	assertThat(nights, is(111L));
    }
