...
public class NumberOfNightsCounterTest2018August {
	...
	final long nights = new NumberOfNightsCounter(month,
		new BookingsByOrigin<>(data.getBookings()).getAirbnbBookings()).setIgnorePaymentDate(true)
			.countNights();
	assertThat(nights, is(6L));
    }

    @Test
    public void testAll01() {
	long nights = new NumberOfNightsCounter(month, new BookingsByOrigin<>(data.getBookings()).getAllBookings(true))
		.setIgnorePaymentDate(false).countNights();
	assertThat(nights, is(6 + 90L));
