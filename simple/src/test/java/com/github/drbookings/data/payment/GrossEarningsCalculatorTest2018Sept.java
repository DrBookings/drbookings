...
public class GrossEarningsCalculatorTest2018Sept {
	...
	assertThat(a.with(Monetary.getDefaultRounding()), is(Payments.createMondary(6666.13)));
    }

    @Test
    public void testOther01() {
	final MonetaryAmount a = new DefaultGrossPaymentsSupplier(month)
		.apply(new BookingsByOrigin<>(data.getBookings()).getOtherBookings());
	assertThat(a.with(Monetary.getDefaultRounding()), is(Payments.createMondary(350)));
    }

    @Test
    public void testAirbnb01() {
	final MonetaryAmount a = new DefaultGrossPaymentsSupplier(month)
		.apply(new BookingsByOrigin<>(data.getBookings()).getAirbnbBookings());
	assertThat(a.with(Monetary.getDefaultRounding()), is(Payments.createMondary(187.13)));
    }
