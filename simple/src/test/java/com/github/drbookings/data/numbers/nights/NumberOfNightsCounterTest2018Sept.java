...
public class NumberOfNightsCounterTest2018Sept {
	...
	final long nights = new NumberOfNightsCounter(month,
		new BookingsByOrigin<>(data.getBookings()).getBookingBookings()).setIgnorePaymentDate(false)
			.countNights();
	assertThat(nights, is(97L - 5 - 1 - 1 - 2));
    }

    @Test
    public void testBookingInvoice01() {
	final long nights = new NumberOfNightsCounter(month,
		new BookingsByOrigin<>(data.getBookings()).getBookingBookings()).setIgnorePaymentDate(false)
			.countNights();
	assertThat(nights, is(97L - 5 - 1 - 1 - 2));
	...
	for (final BookingBean b : new BookingsByOrigin<>(data.getBookings()).getBookingBookings()) {
	    System.err.println(BookingBeans.toString(b) + "\t\t" + new NumberOfNightsCounter(month, b).countNights());
	...
	final long nights = new NumberOfNightsCounter(month,
		new BookingsByOrigin<>(data.getBookings()).getBookingBookings()).setIgnorePaymentDate(true)
			.countNights();
	assertThat(nights, is(108L - 5 - 1 - 1 - 2 - 4 - 4 - 0 - 0));
    }
	...
	final long nights = new NumberOfNightsCounter(month,
		new BookingsByOrigin<>(data.getBookings()).getAirbnbBookings()).setIgnorePaymentDate(false)
			.countNights();
	...
	final long nights = new NumberOfNightsCounter(month,
		new BookingsByOrigin<>(data.getBookings()).getOtherBookings()).setIgnorePaymentDate(false)
			.countNights();
