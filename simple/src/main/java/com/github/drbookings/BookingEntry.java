...
public class BookingEntry extends DateRoomEntry<BookingBean> implements NetEarningsProvider, GrossEarningsProvider, EarningsProvider, IBooking, RoomDateProvider {
	...
	public static Callback<BookingEntry, Observable[]> extractor() {
		...
		return param -> new Observable[]{param.netEarningsProperty(), param.getElement().paymentDoneProperty(), param.getElement().welcomeMailSendProperty()};
	}

	public static Set<String> guestNameView(final Collection<? extends BookingEntry> bookings) {

		return bookings.stream().map(b -> b.getElement().getGuest().getName()).collect(Collectors.toCollection(LinkedHashSet::new));
	}

	public static Set<String> originView(final List<BookingEntry> bookings) {

		return bookings.stream().map(b -> b.getElement().getBookingOrigin().getName()).collect(Collectors.toCollection(LinkedHashSet::new));
	}
	...
	public BookingEntry(final LocalDate date, final BookingBean booking) {
		...
		grossEarningsProperty().bind(Bindings.createObjectBinding(calculateGrossEarnings(), getElement().grossEarningsProperty(), SettingsManager.getInstance().showNetEarningsProperty()));
		netEarningsProperty().bind(Bindings.createObjectBinding(calculateNetEarnings(), getElement().netEarningsProperty()));
	}
