...
public class CleaningPlanController implements Initializable {
	...
	content.itemsProperty().bind(Bindings.createObjectBinding(updateTable(), manager.cleaningsChangedProperty()));
	content.sort();
    }

    private Callable<ObservableList<CleaningEntry>> updateTable() {
	return () -> FXCollections.observableArrayList(manager.getCleaningEntries().stream()
		.filter(e -> e.getDate()
			.isAfter(LocalDate.now().minusDays(SettingsManager.getInstance().getCleaningPlanLookBehind())))
