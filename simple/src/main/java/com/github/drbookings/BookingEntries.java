...
public class BookingEntries {
	...
	long result = 0;
	result += countBookings(bo.getAirbnbElements());
	result += countBookings(bo.getBookingElements());
	if (all) {
	    result += countNights(bo.getOtherElements());
	}
	return result;
    }

    public static int countBookings(final Collection<? extends BookingEntry> bookings) {

	final Set<BookingBean> distinctBookings = getBookings(bookings);
	final int countSplitBookings = (int) distinctBookings.stream().filter(BookingBean::isSplitBooking).count();
	...
	long result = 0;
	result += countNightsAirbnb(bo.getAirbnbElements());
	...
	return bookings.stream().map(b -> b.getElement()).collect(Collectors.toCollection(LinkedHashSet::new));
    }

    public static double getCleaningFees(final Collection<? extends BookingEntry> bookings) {

	// return getCleanings(bookings).stream().mapToDouble(value ->
	// value.getBooking().getCleaningFees()).sum();
	...
	// Collection<CleaningEntry> result = bookings.stream().filter(b ->
	// b.getElement().getCleaning() != null).map(e ->
	// e.getElement().getCleaning()).collect(Collectors.toSet());
	//
    ...
    public static float getEarningsAirbnb(final Collection<? extends BookingEntry> bookings, final boolean netEarnings,
	    final boolean paymentDone) {

	final BookingsByOrigin<BookingEntry> bo = new BookingsByOrigin<>(bookings);
	final AirbnbEarningsCalculator aec = new AirbnbEarningsCalculator().filterForNetEarnings(netEarnings)
		.filterForPaymentDone(paymentDone);
	...
	for (final Entry<BookingBean, Collection<BookingEntry>> en : map.entrySet()) {
		...
		if (logger.isInfoEnabled()) {
		    logger.info("Split-payment (" + String.format("%5.2f", earnings) + ") for " + en.getKey()
			    + ", days of month " + daysCurrentMonth);
		}
		result += earnings;
	    } else {
    ...
    public static Collection<BookingBean> toBookings(final Collection<? extends BookingEntry> bookingEntries) {

	return toBookings(bookingEntries.stream());
    }

    public static Collection<BookingBean> toBookings(final Stream<? extends BookingEntry> bookingEntries) {
